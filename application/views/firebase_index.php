<html>
    <head>
        <title>TMN Assist| Firebase Cloud Messaging</title>
        <meta charset="UTF-8">
    ...
    <body>
        ...
        require_once APPPATH . '/helpers/firebase.php';
        require_once APPPATH . '/helpers/push.php';
        ...
        $push->setTitle($title);
        $push->setNotifTitle($title);
        $push->setMessage($message);
        $push->setNotifBody($message);
        ...
        $nofitication_payload = array();
        $notification_payload['title'] = $push->getNotifTitle();
        $notification_payload['body'] = $push->getNotifBody();
        ...
        if ($push_type == 'topic') {
            $json = $push->getPush();
            $response = $firebase->sendToTopic('announcement', $notification_payload, $json);
        } else if ($push_type == 'individual') {
            $json = $push->getPush();
            $regId = isset($_GET['regId']) ? $_GET['regId'] : '';
            $response = $firebase->send($regId, $notification_payload, $json);
        }
        ?>
        <div class="container">
            <div class="fl_window">
...
<label for="include_image" class="pure-checkbox">
            ...
            <form class="pure-form pure-form-stacked" method="get">
                <fieldset>
                    <legend>Send to Topic `announcement`</legend>
